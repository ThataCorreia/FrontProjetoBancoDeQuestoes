...
function cadastraQuestoes() {
  ...
  fetch(
    "http://localhost:8080/requestMapping/postMapping", // insera o endereço da sua api, se local, permaneça com o localHost, se na nuvem, utilize o endereço de lá
    {
      headers: {
...
function obterQuestoes() {
  ...
  const params = new URLSearchParams({
    // transforma nosso parametro em um URL
  ...
  fetch(`http://localhost:8080/requestMapping/getMapping?${params}`, {
    ...
    .then((Questao) => {
      if (Object.keys(Questao).length == 0) {
        passarTextoParaHtml(
          "h4",
          `Questão não encontrada no banco de dados, adicione ou procure outra questão.`
        );
      } else {
